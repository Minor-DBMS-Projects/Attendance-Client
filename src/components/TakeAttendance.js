...
function TakeAttendance(props) {
  ...
  if (data.classes[0].classType === "L") {
    ...
    classType = (
      <div className="form-group col-md-6">
        <label className="grey-text font-weight-light">Class Type:</label>
        <select
          id="classType"
          className="form-control"
          name="classType"
  ...
  function handleChange(event) {
    ...
    setPresent(
      data.students.map((eachStudent, index) =>
        eachStudent.roll_no === roll_no ? !present[index] : present[index]
      )
    );
  }
            ...
            {data.students.map((student, index) => {
              return (
                <tr
                  key={student.roll_no}
                  style={{
                    backgroundColor: present[index] ? "#90ee90" : "#ffcccb",
                  }}
                >
                  ...
                  <td>
                    <input
                      ...
                      value={present[index] ? "present" : "absent"}
