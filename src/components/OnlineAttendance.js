        ...
        if (subjects === undefined || subjects.length === 0) {
          ...
          setSubjectOptions(
            subjects.map((eachSubject) => (
              <option key={eachSubject[0]} value={JSON.stringify(eachSubject)}>
      ...
      if (parseInt(batch) < 76) {
        if (parseInt(batch) < 75) {
          ...
          setProgramOptions(
            programs
              .filter(
                (eachProgram) => eachProgram !== "BCH" && eachProgram !== "BEX"
              )
              .map((eachProgram) => (
                <option key={eachProgram} value={eachProgram}>
                  {eachProgram}
        ...
        (studentList) => {
          if (studentList === undefined || studentList.length === 0) {
            ...
            for (var property in studentDetails) {
              ...
              var encodedValue = encodeURIComponent(studentDetails[property]);
            ...
            axios
              ...
              .then(() => {
                if (section.length === 2) {
                  APICall.fetchStudents(program, batch, section.charAt(1)).then(
                    (nextStudentList) => {
                      const nextStudentDetails = {
                        batch: batch,
                        program: program,
                        section: section.charAt(1),
                        students: JSON.stringify(nextStudentList),
                      };
                      ...
                      axios
                        .post("/backend/class/add-class", nextStudentFormBody, {
                          headers: {
                            "Content-Type":
                              "application/x-www-form-urlencoded;charset=UTF-8",
                          },
              ...
              .then(() => {
                ...
                for (var property in subjectDetails) {
                  ...
                  subjectFormBody.push(encodedKey + "=" + encodedValue);
                }
                subjectFormBody = subjectFormBody.join("&");
                axios
                  .post("/backend/subject/add-subject", subjectFormBody, {
                    headers: {
                      "Content-Type":
                        "application/x-www-form-urlencoded;charset=UTF-8",
                    },
              ...
              .catch((err) => alert("Unexpected error while adding classes"));
          }
        }
      );
    }
          ...
          <MDBRow>
            <MDBCol md="4" className="mb-3">
              ...
              <div className="invalid-feedback d-block">
                {!isBatchValid ? "Please provide a valid batch." : ""}
              </div>
            </MDBCol>
            <MDBCol md="4" className="mb-3">
              <label
